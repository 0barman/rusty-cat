...
| Bounded in-memory binary GET | Yes | `MeowClient::try_enqueue_binary_task(...)` returns `Bytes` plus optional `Content-Type`. It has an isolated runtime/client/callback path, fixed concurrency 2, capacity 1024, cancel-only control, and is excluded from `snapshot()`. |
...
| `try_enqueue_binary_task(task, complete_cb)` | Download a small bounded response into memory. | Synchronous fail-fast admission; HTTP runs on isolated resources. The callback receives `BinaryDownloadOutput` (`Bytes` + optional `Content-Type`). Supports cancel only, is absent from snapshot, runs at concurrency 2, and keeps at most 1024 accepted tasks through callback completion. |
