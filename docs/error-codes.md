...
| `109` | `ResponseStatusError` | The HTTP response status was not the expected success status. | Server returned `4xx`/`5xx` (for example `403` on an expired presigned URL). | Inspect `msg()`; regenerate URLs/credentials or fix the request; not auto-retried. |
...
- `106` (`HttpError`) and other transient transport failures are retried
  automatically within the task's retry budget — see
  [Retry and transient errors](../README.md#retry-and-transient-errors).
- For restart/crash recovery semantics per error (for example resuming after
  `121`/`122`), see [Resuming after a restart](resume-after-restart.md).
